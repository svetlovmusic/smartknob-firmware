...
void ErrorHandlingFlow::handleEvent(WiFiEvent event)
{
    ...
    case SK_MQTT_RETRY_LIMIT_REACHED:
        if (!WiFi.isConnected())
        {
            sprintf(ip_data, "%s", "https://192.168.4.1/?mqtt"); // always the same
        }
        else
        {
            // TODO: look into how to store and retrieve ip in a better way.
            sprintf(ip_data, "https://%s/?mqtt", WiFi.localIP().toString().c_str());
        }
    ...
    case SK_WIFI_STA_RETRY_LIMIT_REACHED:
        if (!WiFi.isConnected())
        {
            sprintf(ip_data, "%s", "https://192.168.4.1/?wifi"); // always the same
        }
        else
        {
            // TODO: look into how to store and retrieve ip in a better way.
            sprintf(ip_data, "https://%s/?wifi", WiFi.localIP().toString().c_str());
        }
        error_page->setQr(ip_data);
